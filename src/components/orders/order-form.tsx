...
import type { Clinic, Order, OrderInput, Patient } from "@/lib/api";
...
export function OrderForm({
  ...
  const call = useApi();
  const patients = useQuery({ queryKey: ["patients", ""], queryFn: () => call<Patient[]>("/api/patients") });
  const clinics = useQuery({ queryKey: ["clinics"], queryFn: () => call<Clinic[]>("/api/clinics") });
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select
                  items={Object.fromEntries((patients.data ?? []).map((p) => [p.id, `${p.lastName}, ${p.firstName}`]))}
                  value={field.value}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select
                  items={{ [NONE]: t("orders.form.selectClinic"), ...Object.fromEntries((clinics.data ?? []).map((c) => [c.id, c.name])) }}
                  value={field.value}
                  onValueChange={field.onChange}
                >
                  <FormControl>
